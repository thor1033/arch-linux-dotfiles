...
const BarLauncher = BarWidget({
    class_name: "launcher",
    eventbox: {
        on_primary_click_release: () => Utils.execAsync(["pkill", "rofi"])
        ...
        setup: widget => {
            widget.hook(Hyprland, widget => {
                ...
                widget.children = workspaces.map(ws => Widget.Button({
                    ...
                    on_primary_click_release: () => Utils.execAsync(`hyprctl dispatch workspace ${ws.id}`),
                }));
            });
        },
    },
        ...
        box: {
            children: SystemTray.bind("items").transform(items => {
                return items.map(item => Widget.Button({
                    on_primary_click_release: () => item.openMenu(null),
                    child: Widget.Icon({
                        icon: item.bind("icon"),
                        size: 18,
                    }),
...
const BarSysInfo = BarWidget({
    class_name: "sysinfo",
    eventbox: {
        on_primary_click_release: () => BarSysTray.reveal_child = !BarSysTray.reveal_child,
        on_middle_click_release: () => App.Inspector(),
    },
    box: {
        spacing: 12,
        children: [
...
const BarPlayer = BarWidget({
    ...
    eventbox: {
        ...
        on_primary_click_release: () => WindowHandler.toggle_window("player"),
        on_secondary_click: () => {
...
const BarMedia = BarWidget({
    ...
    eventbox: {
        ...
        on_primary_click_release: () => WindowHandler.toggle_window("media"),
    },
    box: {
        spacing: 10,
        children: [
...
const BarclockInfo = Widget.EventBox({
    class_name: "clock",
...
const BarInfo = BarWidget({
    class_name: "info",
    eventbox: {
        on_primary_click_release: () => WindowHandler.toggle_window("glance"),
    },
    box: {
        spacing: 10,
        children: [
...
const BarPower = BarWidget({
    class_name: "power",
    eventbox: {
        on_primary_click_release: () => WindowHandler.toggle_window("powermenu"),
